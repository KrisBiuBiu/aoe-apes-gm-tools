...
module.exports.regist = function (dbproxy_register) {
	function user_vip(req, res, callback) {
		...
		if (dburl) {
			...
			if (!isNaN(pageNum) && !isNaN(page_id) && !isNaN(region) && !isNaN(serverid) && starttime.isValid() && endtime.isValid() && starttime.valueOf() < endtime.valueOf()) {
				mongoClient.connect(dburl, function (err, db) {
					...
					db.collection('user_vip', { strict: true }, function (err, coll) {
						if (err == null) {
							if (page_id < 1) {
								page_id = 1
							}
							...
							var resultMap = {'user_id': 1,'vip_level':1, 'channel': 1,'created_date':1,'project_id':1, _id: 0 };
	...
	dbproxy_register["user_vip"] = user_vip;
}
